...
export const PUT: APIRoute = async ({ request }) => {
  ...
  const stmt = env.DB.prepare('UPDATE apps SET display_order = ? WHERE id = ? AND user_id = ?');
  await env.DB.batch(orders.map((o) => stmt.bind(o.display_order, o.id, user.userId)));
