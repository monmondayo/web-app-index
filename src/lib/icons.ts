...
export const TECH_DESCRIPTIONS: Record<string, string> = {
  ...
  'Gradio': 'PythonだけでAIアプリのWeb UIを構築できるライブラリ',
  'Replicate': 'クラウド上のAIモデルをAPI経由で実行できるプラットフォーム',
  'Hugging Face': 'AIモデル・データセット・アプリを公開、実行できるプラットフォーム',
  'Android': 'Googleが提供するモバイルOS・アプリ実行プラットフォーム',
  'Kotlin': 'Android開発で広く使われる型安全なプログラミング言語',
  'Jetpack Compose': 'KotlinでAndroid UIを宣言的に構築するツールキット',
  'CameraX': 'Androidのカメラ機能を扱いやすくするJetpackライブラリ',
